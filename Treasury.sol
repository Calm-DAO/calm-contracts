...
interface ICALMERC20 is IERC20Mintable, IERC20 {
...
contract CalmTreasury is Ownable {
  using LowGasSafeMath for uint256;
  ...
  ICALMERC20 public immutable Calm;
  ...
  /**
        @notice allow approved address to deposit an asset for Calm
        @param _amount uint
  ...
  function deposit(
    ...
    // mint Calm needed and store amount of rewards for distribution

    uint256 send_ = value.sub(_profit);

    limitRequirements(msg.sender, send_);
    Calm.mint(msg.sender, send_);
  ...
  /**
        @notice allow approved address to burn Calm for reserves
        @param _amount uint
        @param _token address
     */
  function withdraw(uint256 _amount, address _token) external {
    ...
    uint256 value = valueOf(_token, _amount);
    Calm.burnFrom(msg.sender, value);
  ...
  function incurDebt(uint256 _amount, address _token) external {
    ...
    uint256 maximumDebt = sCalm.balanceOf(msg.sender); // Can only borrow against sOHM held
  ...
  /**
        @notice allow approved address to repay borrowed reserves with Calm
        @param _amount uint
     */
  function repayDebtWithCalm(uint256 _amount) external {
    require(isDebtor[msg.sender], "Not approved as debtor");
    require(isReserveSpender[msg.sender], "Not approved as spender");

    Calm.burnFrom(msg.sender, _amount);

    debtorBalance[msg.sender] = debtorBalance[msg.sender].sub(_amount);
    totalDebt = totalDebt.sub(_amount);

    emit RepayDebt(msg.sender, address(Calm), _amount, _amount);
  }
  ...
  function mintRewards(address _recipient, uint256 _amount) external {
    ...
    console.log("Total reserves: ", totalReserves);
    console.log("Calm total supply: ", Calm.totalSupply());
    console.log("Total debt: ", totalDebt);

    require(_amount <= excessReserves(), "Insufficient reserves");

    limitRequirements(msg.sender, _amount);
    Calm.mint(_recipient, _amount);
  ...
  function excessReserves() public view returns (uint256) {
    return totalReserves.sub(Calm.totalSupply().sub(totalDebt));
  }
  ...
  /**
        @notice returns Calm valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    ...
    returns (uint256 value_)
  {
    if (isReserveToken[_token]) {
      ...
      // convert amount to match Calm decimals
      value_ = _amount.mul(10**Calm.decimals()).div(

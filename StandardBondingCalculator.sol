...
contract CalmBondingCalculator is IBondingCalculator {
    ...
    function markdown(address _pair) external view returns (uint256) {
        ...
        uint256 reserve;
        if (IUniswapV2Pair(_pair).token0() == address(Calm)) {
            reserve = reserve1;
        } else {
            require(
                IUniswapV2Pair(_pair).token1() == address(Calm),
                "not a Calm lp pair"
            );
            reserve = reserve0;
        }
        return reserve.mul(2 * (10**Calm.decimals())).div(getTotalValue(_pair));
    }
}

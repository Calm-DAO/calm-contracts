...
contract PreCalmToken is ERC20Permit, VaultOwned {
  using LowGasSafeMath for uint256;
  ...
  constructor() ERC20("PreCalm", "pCALM", 18) {
  ...
  /*
   * PreCalm is a private option token. The option comes with the limitation that is can only be traded with existing holders.
  ...
  function _beforeTokenTransfer(
    ...
    if (requireSellerApproval) {
      require(
        ...
        "Account not approved to transfer pCALM."
      );
    } else {
      console.log(
        "Contract::PreCalmToken::_beforeTokenTransfer:3 Transfers and sales are not limited."
      );
    }
  }

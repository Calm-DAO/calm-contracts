...
contract CalmBondDepository is Ownable {
  ...
  ITreasury public immutable treasury; // mints Calm when receives principle
  ...
  struct Bond {
    uint256 payout; // Calm remaining to be paid
  ...
  function deposit(
    ...
    require(payout >= 10000000, "Bond too small"); // must be > 0.01 Calm ( underflow protection )
    ...
    uint256 balanceBefore = Calm.balanceOf(address(this));
    /**
            principle is transferred in
            approved and
            deposited into the treasury, returning (_amount - profit) Calm
         */
    ...
    if (fee != 0) {
      ...
      Calm.safeTransfer(DAO, fee);
    }
    // require(
    //   balanceBefore.add(profit) == Calm.balanceOf(address(this)),
    //   "Not enough Calm to cover profit"
    // );
  ...
  function stakeOrSend(
    ...
    if (!_stake) {
      ...
      Calm.transfer(_recipient, _amount); // send payout
    } else {
      // if user wants to stake
      if (useHelper) {
        // use if staking warmup is 0
        Calm.approve(address(stakingHelper), _amount);
        stakingHelper.stake(_amount, _recipient);
      } else {
        Calm.approve(address(staking), _amount);
  ...
  function maxPayout() public view returns (uint256) {
    return Calm.totalSupply().mul(terms.maxPayout) / 100000;
  }
  ...
  /**
   *  @notice calculate current ratio of debt to Calm supply
   *  @return debtRatio_ uint
   */
  function debtRatio() public view returns (uint256 debtRatio_) {
    uint256 supply = Calm.totalSupply();
    debtRatio_ =
  ...
  /**
   *  @notice calculate amount of Calm available for claim by depositor
   *  @param _depositor address
  ...
  /**
   *  @notice allow anyone to send lost tokens (excluding principle or Calm) to the DAO
   *  @return bool
   */
  function recoverLostToken(IERC20 _token) external returns (bool) {
    require(_token != Calm, "NAT");
